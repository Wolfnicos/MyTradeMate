...
final class StrategyManager: ObservableObject {
    ...
    func enableStrategy(named name: String) {
        if let strategy = strategies.first(where: { $0.name == name }) {
    ...
    func updateStrategyWeight(named name: String, weight: Double) {
        if let strategy = strategies.first(where: { $0.name == name }) {
            strategy.weight = max(0.1, min(2.0, weight))
            saveConfiguration()
    ...
    private func loadConfiguration() {
        ...
        do {
            ...
            for strategy in strategies {
                strategy.isEnabled = config.enabledStrategies.contains(strategy.name)
                strategy.weight = config.strategyWeights[strategy.name] ?? 1.0
            }
